...
#@Thomas: Add the vulcan directory to the system path to load in the modules store, build_atm, op, and chem_funs
sys.path.append('/STER/thomasko/VULCAN_TK/vulcan_python_scripts/') #@Thomas
# import the configuration inputs from the current working directory
import vulcan_cfg
...
if len(sys.argv) < 2 or sys.argv[1] != '-n': 
    ...
    #@Thomas: Add a full path to the make_chem_funs.py script
    full_path = vulcan_cfg.vulcan_dir + 'vulcan_python_scripts/make_chem_funs.py' #@Thomas
    os.system(python_executable + ' ' + full_path)
