...
# input:
vulcan_dir = '/STER/thomasko/VULCAN_TK/' #@Thomas
network = vulcan_dir + 'thermo/SNCHO_photo_network.txt'
use_lowT_limit_rates = False
gibbs_text = vulcan_dir + 'thermo/gibbs_text.txt' # (all the nasa9 files must be placed in the folder: thermo/NASA9/)
cross_folder = vulcan_dir + 'thermo/photo_cross/'
com_file = vulcan_dir + 'thermo/all_compose.txt'
atm_file = vulcan_dir + 'atm/VULCAN_input_files_WASP107b/WASP-107b_picaso_Tint300_10Z_VULCAN.apt' # TP and Kzz (optional) file
sflux_file = vulcan_dir + 'atm/stellar_flux/VPL_solar.txt' # sflux-HD189_B2020.txt This is the flux density at the stellar surface
...
output_dir = '/STER/thomasko/VULCAN_output/testing/'
plot_dir = 'plot/'
...
# customized elemental abundance (only read when use_solar = False) --> Currently set to 10x solar
O_H = 0.0044789563477    #10x solar
C_H = 0.0024613695853     #10x solar
N_H = 0.00061826808643  #10x solar
S_H = 0.0001205526689    #10x solar
He_H = 0.07783534053     #10x solar
ini_mix = 'EQ' # Options: 'EQ', 'const_mix', 'vulcan_ini', 'table' (for 'vulcan_ini, the T-P grids have to be exactly the same)
fastchem_met_scale = 10 # scaling factor for other elements in fastchem (e.g., if fastchem_met_scale = 0.1, other elements such as Si and Mg will take 0.1 solar values)
...
r_star = 0.66 # stellar radius in solar radius
Rp = 10.54 * 6.378E8 # Planetary radius (cm) (for computing gravity)
orbit_radius = 0.05500 # planet-star distance in A.U.
...
gs = 269.28         # surface gravity (cm/s^2)  (HD189:2140  HD209:936)
...
plot_spec = ['H2O', 'H', 'CH4', 'CO', 'CO2', 'C2H2', 'HCN', 'NH3', 'SO2' ]
# output:
